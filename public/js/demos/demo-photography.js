...
(function($) {
					...
					setTimeout(function() {
						...
						self.$wrapper.on( 'layoutComplete', function( laidOutItems ) {
							...
							self.$wrapper.find('.item-appended').each(function(){
								...
								// Add images
								$('#thumbGalleryDetail').owlCarousel().trigger('add.1owl.carousel', [clone]);
								$('#thumbGalleryThumbs').owlCarousel().trigger('add.1owl.carousel', [cloneThumb]);
	...
	*/
	var clickedItem,
		...
		callbacks: {
			open: function(){
				$('#thumbGalleryDetail').owlCarousel().trigger('refresh.1owl.carousel');
				$('#thumbGalleryDetail').owlCarousel().trigger('to.1owl.carousel', [clickedItem, 0]);
	...
	if( $('#photographyLightbox').get(0) ) {
		...
		$thumbGalleryDetail
			...
			.on('changed.owl.carousel', function(e) {
				if (!flag) {
					flag = true;
					$thumbGalleryThumbs.trigger('to.1owl.carousel', [e.item.index-1, duration, true]);
		...
		$thumbGalleryThumbs
			...
			.on('click', '.owl-item', function() {
				$thumbGalleryDetail.trigger('to.1owl.carousel', [$(this).index(), duration, true]);

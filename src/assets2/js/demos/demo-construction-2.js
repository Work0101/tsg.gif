...
(function ($) {
	...
	$customThumbGalleryDetail
		...
		.on('changed.owl.carousel', function(e) {
			if (!flag) {
				flag = true;
				$customThumbGalleryThumbs.trigger('to.1owl.carousel', [e.item.index-1, duration, true]);
				flag = false;
			}
		});
	...
	$customThumbGalleryThumbs
		...
		.on('click', '.owl-item', function() {
			$customThumbGalleryDetail.trigger('to.1owl.carousel', [$(this).index(), duration, true]);
		})
		.on('changed.owl.carousel', function(e) {
			if (!flag) {
				flag = true;
				$customThumbGalleryDetail.trigger('to.1owl.carousel', [e.item.index, duration, true]);
				flag = false;
			}
		});

        ...
        <div className={"new-container"} style={{
            ...
            <div className={'new-container'}
                ...
                <h3 className={"text-color-dark"}
                    style={{marginBottom: "30px", marginTop: "20px", fontSize: "22px"}}> Sign In</h3>
                <div className={'d-flex align-items-center flex-column'} style={{width: "90%"}}>
                    <form onSubmit={handleSubmit((value) => console.log(value))} className={"form-container"}>
                        ...
                        <FormInput params={paramsForSelectType} isValid={isValid} selectType={selectType} watch={watch}
                                   setValue={setValue}
                                   register={register} errors={errors} trigger={trigger} touchedFields={touchedFields}
                                   control={control} resetField={resetField}/>
                        <PasswordInput register={register} isValid={isValid} errors={errors} selectType={selectType}
                                       type={"password"} trigger={trigger} touchedFields={touchedFields}
                                       control={control}/>
                        <div style={{
                            display: "flex",
                            width: "100%",
                            ...
                            <div style={{position: "relative"}}>
                                <div className={`checkbox-container ${isChecked ? 'active' : ""} forgot-password`}
                                     onClick={handleCheckboxChange}>
                                    <div className={`checkbox--svg-container ${isChecked ? 'active' : ""}`}>
                                        <ImCheckmark/>
                                    </div>
                                    <div className={ `checkbox--text-container ${isChecked?'active':""}`}>Sign In (31 days)</div>
                                </div>
                            </div>
                        </div>
